#!/usr/bin/env bash
# instalador_mmdvm.sh - Puente MMDVM (serial) para ZetronPOC (Ubuntu Server / PC x86_64)
# Conecta la placa MMDVM por USB-TTL y deja POCSAG nativo (sin .wav, sin GPIO).
# Reemplaza la transmision por audio por comunicacion serial al encoder MMDVMHost.
#
# Uso:  curl -fsSL https://raw.githubusercontent.com/gatoambroggio/ubntpagingsystem/main/instalador_mmdvm.sh | sudo bash
#   o:  sudo bash instalador_mmdvm.sh
#   o:  sudo bash instalador_mmdvm.sh --callsign=LU1ABC --port=/dev/ttyUSB0 --freq=433.800
# ============================================================================
set -euo pipefail

MMDVM_CALLSIGN="LU1ABC"
MMDVM_PORT="/dev/ttyUSB0"
MMDVM_BAUD="115200"
MMDVM_FREQ="433.800"
MMDVM_DUPLEX="0"
MMDVM_TXINVERT="1"
...
echo "=================================================="
echo " MMDVM Bridge para ZetronPOC  (Ubuntu Server / PC)"
echo "   Placa : $MMDVM_PORT @ $MMDVM_BAUD baud"
echo "   Frec  : $MMDVM_FREQ MHz ($FREQ_HZ Hz)"
echo "   Call  : $MMDVM_CALLSIGN   RC: $MMDVM_RCPORT"
echo "=================================================="

note "0/8 Verificando instalacion base de ZetronPOC..."
...
apt-get update -y
apt-get install -y build-essential git libudev-dev curl ca-certificates netcat-openbsd socat nlohmann-json3-dev libmosquitto-dev || warn "Algun paquete pudo faltar"
...
# Placa MMDVM por puerto serie (sin Raspberry, sin .wav)
...
note "4/8 Creando servicio systemd mmdvmhost..."
...
systemctl enable --now mmdvmhost
sleep 2
systemctl is-active --quiet mmdvmhost && log "mmdvmhost corriendo" || warn "mmdvmhost no inicio. Conecta la placa y revisa: journalctl -u mmdvmhost -f"

note "5/8 Creando puente POCSAG -> MMDVM (RemoteCommand)..."
...
note "6/8 Adaptando el AGI para modo MMDVM..."
patch_agi() {
  local f="$1"
  [[ -f "$f" ]] || return
...
note "7/8 Seteando modo MMDVM en la base de ZetronPOC..."
...
note "8/8 Chequeo final..."
echo "  Estado del modem:"
systemctl is-active mmdvmhost 2>/dev/null | sed 's/^/    /'
echo "  Ultimas lineas:"
journalctl -u mmdvmhost -n 5 --no-pager 2>/dev/null | sed 's/^/    /' || true
echo ""
log "Puente MMDVM instalado para ZetronPOC."
echo ""
echo "  PROXIMOS PASOS:"
echo "    1) Conecta la placa MMDVM por USB-TTL -> ${MMDVM_PORT}"
echo "    2) Ajusta frecuencia/nivel en el panel admin -> Parametros -> MMDVM"
echo "    3) Prueba local (sin DAPNET, sin .wav):"
echo "       ${TX_SCRIPT} 1234567 'PRUEBA HOSPITAL' 1200"
echo "    4) Logs en vivo:  sudo journalctl -u mmdvmhost -f"
echo ""
echo "  La placa transmite POCSAG nativo. El panel genera el MMDVM.ini y"
echo "  reinicia el servicio al pulsar 'Aplicar a la placa MMDVM'."
