...
#!/usr/bin/env python3
# Codificador POCSAG puro (stdlib). Basado en el algoritmo de faithanalog/pocsag-encoder.
# Genera WAV mono 16-bit NRZ listo para alimentar la entrada de audio de un TX FM.
...
def encode_codeword(msg):
    full = (msg << CRC_BITS) | crc(msg)
    return (full << 1) | parity(full)

def address_offset(address):
    return (address & 0x7) * FRAME_SIZE

def encode_ascii(initial_offset, s, out):
    cur = 0; nbits = 0; pos = initial_offset
    for c in s:
        for i in range(TEXT_BITS_PER_CHAR):
            cur = (cur << 1) | ((ord(c) >> i) & 1)
            nbits += 1
            if nbits == TEXT_BITS_PER_WORD:
                out.append(encode_codeword(cur | FLAG_MESSAGE))
                cur = 0; nbits = 0; pos += 1
                if pos == BATCH_SIZE: out.append(SYNC); pos = 0
    if nbits > 0:
        cur <<= (20 - nbits)
        out.append(encode_codeword(cur | FLAG_MESSAGE))
        pos += 1
...
def encode_transmission(address, message):
    out = []
    for _ in range(PREAMBLE_LENGTH // 32): out.append(0xAAAAAAAA)
    start = len(out)
    out.append(SYNC)
    for _ in range(address_offset(address)): out.append(IDLE)
    out.append(encode_codeword(((address >> 3) << 2) | FLAG_TEXT_DATA))
    encode_ascii(address_offset(address) + 1, message, out)
    out.append(IDLE)
    written = len(out) - start
    pad = (BATCH_SIZE + 1) - (written % (BATCH_SIZE + 1))
    for _ in range(pad): out.append(IDLE)
    return out

def modulate(codewords, baud, sample_rate):
    spb = sample_rate // baud
    one = struct.pack('<h', -30000) * spb
    zero = struct.pack('<h', 30000) * spb
    chunks = []
    for w in codewords:
...
def main():
    if len(sys.argv) != 5:
        print("Uso: pocsag_gen.py <cap> <msg> <baud> <out.wav>", file=sys.stderr); return 1
    cap, msg, baud, out_path = sys.argv[1], sys.argv[2], int(sys.argv[3]), sys.argv[4]
    if baud not in (512, 1200, 2400): baud = 1200
    sample_rate = 38400  # divisible por 512, 1200 y 2400
    if sample_rate % baud != 0: sample_rate = baud * 32
    codewords = encode_transmission(int(cap), msg)
    data = modulate(codewords, baud, sample_rate)
...
#!/usr/bin/env python3
import os, sys, json, csv, io, subprocess, shutil
...
<button onclick="tab('pbx',event)">PBX</button>
...
<div class="tab" id="t-pbx"><div class="card"><h2>Gestion del PBX (FreePBX/Asterisk)</h2>
<div style="display:flex;gap:.4rem;flex-wrap:wrap;margin-bottom:.8rem">
<button class="btn" onclick="pbx('status')">Estado</button>
<button class="btn" onclick="pbx('peers')">Endpoints</button>
<button class="btn" onclick="pbx('channels')">Canales</button>
<button class="btn" onclick="pbx('uptime')">Uptime</button>
<button class="btn-warn" onclick="pbxReload()">Recargar config</button>
<button class="btn-del" onclick="pbxRestart()">Reiniciar PBX</button>
</div>
<pre id="pbx_out">—</pre>
</div></div>
...
async function enviar(){const codigo=document.getElementById('e_cod').value,mensaje=document.getElementById('e_msg').value.trim();if(!codigo||!mensaje){document.getElementById('e_res').innerHTML='<span class="badge err">Falta codigo o mensaje</span>';return;}
...
async function pbx(cmd){const r=await g('/api/pbx?cmd='+cmd);document.getElementById('pbx_out').textContent=r.salida||r.error||'';}
async function pbxReload(){if(!confirm('Recargar configuracion del PBX?'))return;const r=await fetch('/api/pbx/reload',{method:'POST'}).then(r=>r.json());document.getElementById('pbx_out').textContent=r.salida||'';}
async function pbxRestart(){if(!confirm('Reiniciar PBX? Se cortaran llamadas activas.'))return;const r=await fetch('/api/pbx/restart',{method:'POST'}).then(r=>r.json());document.getElementById('pbx_out').textContent=r.salida||'';}
async function health(){try{const h=await g('/api/health');document.getElementById('h').textContent=h.status==='ok'?'en linea':'caido';}catch(e){document.getElementById('h').textContent='caido';}}
...
cat <<EOF
  ...
  Panel web POCSAG:  http://<servidor>:8080/
